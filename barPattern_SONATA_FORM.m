...
for song_no = 4%:length(index)
clearvars -except index song_no SIZE; close all; clc;
...
% subplot(1,2,1);
%     paramVisScp = [];
%     [h_fig_scapeplot,x_axis,y_axis] = visualizeScapePlot(fitness_matrix,paramVisScp);hold all;
%     set(gca, 'FontSize', SIZE, 'FontName','Times New Roman');
% %     plot(point_x_pos,point_y_pos,'o','LineWidth',4,'color',[1, 0.5, 0],'MarkerSize',15);
% 
%     figure;
% subplot(1,2,1);
%     paramVisScp = [];
...
%     hold on;
%     plot(point_x_pos,point_y_pos,'o','LineWidth',2,'color',[1, 0.5, 0],'MarkerSize',15);
%     hold off;
...
%     eval(['save' parameter.fitnessSaveFileName(1:end-1) ' S_final fitness_info parameter pathFamily']);
    ...
    parameter.title     = filename;
    groundTruth_struct  = structure_annotation_1(filename, 1);
%２０１７１１２５     groundTruth_struct  = structure_annotation([dirAnnotation 'pei_anno/' filename '.txt'], 1);%parseAnnotationFile([dirAnnotation parameter.title '_bar.txt']);
