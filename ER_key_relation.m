...
%   Detailed explanation goes here
     GT_subject  = structure_annotation_1(filename, 2);
...
%             end
            ...
            % k means data
%             X = [X; i PATHFAMILY.shift(:,i)];
% X = [X;PATHFAMILY.shift(:,i) i];
        end
...
%     [idx,C] = kmeans(X,2);
%     PATHFAMILY.kmean_X = X;
%     PATHFAMILY.kmean_idx = idx;
%     PATHFAMILY.kmean_C = C;
%     X1 = X(idx==1,:);
%     X2 = X(idx==2,:);
%     figure;
%     plot(X1(:,1),X1(:,2),'b*','MarkerSize',5); hold all;
...
SHIFTMATRIX.shift_matrix_ori = SHIFTMATRIX.shift_matrix;
   % closing opening
    ...
    E = imerode(tmp,SE); D = imdilate(tmp,SE);
        EAD= figure;
    subplot(5,1,1); plotShift(    tmp, 'Normalize'                                              , pathFamily, GT_subject );
    subplot(5,1,2); plotShift(     E, ['Erode, ' num2str(length(SE.Neighborhood))], pathFamily, GT_subject );
    subplot(5,1,3); plotShift(     D, ['Dilate, ' num2str(length(SE.Neighborhood))], pathFamily, GT_subject );
    subplot(5,1,4); plotShift(Opening, ['Erode -> Dilate, ' num2str(length(SE.Neighborhood))]   , pathFamily, GT_subject );          
    subplot(5,1,5); plotShift(Closing, ['Dilate -> Erode, ' num2str(length(SE.Neighborhood))]   , pathFamily, GT_subject );
    ...
    subplot(2,1,2); plotShift(     OC, ['Opening -> Closing, ' num2str(length(SE.Neighborhood))], pathFamily, GT_subject );
        ...
        f6 = figure;
    subplot(2,2,1); plotShift( tmp, [], pathFamily, GT_subject );
        set(gca, 'FontSize', 20, 'FontName','Times New Roman');

    subplot(2,2,2); plotShift(  OC, [], pathFamily, GT_subject );
        set(gca, 'FontSize', 20, 'FontName','Times New Roman');
...
function plotShift(data, name, pathFamily, GT_subject)
        for i=1:size(data, 2)
            ...
            if ~isempty(idx)
                ...
                plot(pathFamily(1,i), idx,'.','MarkerSize',20,'Color','b'); hold all;
            end
        end
