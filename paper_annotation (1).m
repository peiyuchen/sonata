function RESULT = paper_annotation(filename, type)
...
%           disp(line);
          ...
          if length(line{i,1}) > 9
              ...
              out1 = strcmp(line{i,1}(1:9), 'structure');	% 比較兩字串 str1 和 str2
              ...
              if out1
                  ...
                  data_anno(j,2) = {max(round_05( str2num( data_anno{j,2} ) ), 1)};

                  data_anno(j,3) = {max(round_05( str2num( data_anno{j,3} ) ), 1)};
%                   data_anno(j,2) = {str2num( data_anno{j,2} )};
% 
%                   data_anno(j,3) = {str2num( data_anno{j,3} )};
...
%                       result(E_no, 3:4) = data_anno(j, 2:3);
                  ...
                  % Development
                  if ~strncmp(data_anno{j,4}, 'Exposition', 10) && ~strncmp(data_anno{j,4}, 'Recapitulation', 14) && ~strncmp(data_anno{j,4}, 'Introduction', 12)
...
%{
                  ...
                  if strncmp(data_anno{j,4}, 'Exposition', 10) && strncmp(data_anno{j,5}, 'FirstGroup', 10)     
                      ...
                      GT_subject(row_no).M1_start = data_anno{j, 2};
                      
                      GT_subject(row_no).M1_end = data_anno{j, 3};
                  ...
                  if strncmp(data_anno{j,4}, 'Exposition', 10) && strncmp(data_anno{j,5}, 'SecondGroup', 11)
                      ...
                      GT_subject(row_no).M2_start = data_anno{j, 2};
                      
                      GT_subject(row_no).M2_end = data_anno{j, 3};
