...
%	讀midi檔，轉成相關矩陣 左手 右手 雙手 ，休止符 ，存成.mat檔
    ...
    if nargin < 2, multiProcess = 'modifySkyline'; end  % 多音處理
...
%         P_left   = pitch_Pattern_v3( midiLeft , timeSig);
    ...
    elseif strcmp(multiProcess, 'modifySkyline')
        ...
        P_right  = curve2pattern( pitchInfoT, curveT, timeSig ,1);
        P_left   = curve2pattern( pitchInfoB, curveB, timeSig );

        S.right  = similar_matrix(P_right.barPath, P_right.barPath, 'correlation');
        S.left   = similar_matrix(P_left.barPath , P_left.barPath , 'correlation');
        S.com    = similar_matrix(P_right.barPath, P_left.barPath , 'correlation');
        midiRight= midiALL;
        midiLeft = midiALL; 
