...
%                     pitchCell{currentBar,s:e(end)} = cat(2, pitchCell{currentBar,s:e(end)}, noteBar(k,4));
    ...
    %% 最佳路徑的和弦標記
            ...
            %% 分數一樣的話
                    ...
                    Ans.HighScoreChord{currentBar,k-1} = HighScoreChord';
...
%                                     Dim7.idx(currentBar,Dim7Time)  = k-1;
%                                     Dim7.root{currentBar,k-1}      = HighScoreChord;
%                                     
%                                     step     = 3;
%                                     Dim7Time = Dim7Time + 1;
...
%                       step 3 : Diminished 7th resolution
                    ...
                    if refNextBarRoot
                        idx               = find(Dim7.idx(currentBar-1)~=0);
                        nowRoot           = ~(ceil(mod(Dim7.root{currentBar-1,idx(end)},12)/12))*12 + mod(Dim7.root{currentBar-1,idx(end)},12);
                        nextRoot          = Ans.pitchNo(currentBar,k-1);
                        reRoot            = ~(ceil(mod((nextRoot-1),12)/12))*12 + mod((nextRoot-1),12);
                        refNextBarRoot    = ~refNextBarRoot;
                        currentBar-1
                        nowChordNote      = ~(ceil(mod(Dim7.root{currentBar-1,idx(end)},12)/12))*12 + mod(Dim7.root{currentBar-1,idx(end)},12)
                        nextChordNote     = (mod(template(Ans.templNo(currentBar,k-1),(template(Ans.templNo(currentBar,k-1),:)~=-1)) + Ans.pitchNo(currentBar,k-1)-1,12)+1)'
                        
                        CT = length(intersect(nowChordNote,nextChordNote)) % common tone
                        Dim7.CT{currentBar-1,idx(end)} = CT;
%                         if CT >= 2        
%                            Dim7.CT{currentBar-1,idx(end)} = CT;
%                         elseif CT == 1
%                         
...
%                         end
                        if  any(nowRoot == reRoot)
                            Ans.Chord(currentBar-1,idx(end))     = Dim7.root{currentBar-1,idx(end)}(nowRoot == reRoot);
                            ...
                            step = 4
                            Ans.sameScore(currentBar-1,idx(end)) = step;
                            Untreated = [Untreated currentBar-1];
                            warning(['第 ' num2str(currentBar-1) '小節 - step3 : Fully Diminished 7th 沒有解決'])
                        end 
                    end
                end     
            ...
            % step 3 : Diminished 7th resolution
                ...
                if resolution_idx
                    for t=1:numel(resolution_idx)        
                        nowRoot      = ~(ceil(mod(Dim7.root{currentBar,resolution_idx(t)},12)/12))*12 + mod(Dim7.root{currentBar,resolution_idx(t)},12);
                        if Ans.Chord(currentBar,resolution_idx(t)+1) ~= 0 % 不是小節最後一個chord
                            ...
                            currentBar
                            nextChordNote = (mod(template(Ans.templNo(currentBar,k-1),(template(Ans.templNo(currentBar,k-1),:)~=-1)) + Ans.pitchNo(currentBar,k-1)-1,12)+1)'
                            nowChordNote  = ~(ceil(mod(Dim7.root{currentBar,resolution_idx(end)},12)/12))*12 + mod(Dim7.root{currentBar,resolution_idx(end)},12)
                            CT = length(intersect(nowChordNote,nextChordNote)) % common tone
                            Dim7.CT{currentBar,resolution_idx(end)} = CT;
                            %## end
                            
                            if  any(nowRoot == reRoot)
                                AnsChord = Dim7.root{currentBar,resolution_idx(t)}(nowRoot == reRoot);
                                ...
                                step = 4
                                Ans.sameScore(currentBar,resolution_idx(t)) = step;
                                Untreated = [Untreated currentBar];
