...
function [annoSeg, sideinfo] = parseAnnotationMovementFile(filename)
...
%             myRegExp = '([A-Za-z_]+[\s,]+)?(?<start>\d+\.?\d*[\s,]+)(?<end>\d+\.?\d*[\s,]+)?([\w\:])*(\".*\")?';
            myRegExp = '([A-Za-z_]+[\s,]+)?(?<start>\d+\.?\d*[\s,]+)(?<end>\d+\.?\d*[\s,]+)?(?<start>\d+\.?\d*[\s,]+)(?<end>\d+\.?\d*[\s,]+)?([\w\:])*(\".*\")?';
            myResults = regexp(tline ,myRegExp, 'tokens');

            if ~isempty(myResults)
                myResults   = myResults{:};
                type        = strtrim(myResults{1});
                M1_start    = str2double(strtrim(myResults{2}));
                M1_end      = str2double(strtrim(myResults{3}));
                M2_start    = str2double(strtrim(myResults{4}));
                M2_end      = str2double(strtrim(myResults{5}));
                label       = strtrim(myResults{6});
                ...
                idx = idx + 1;
                annoSeg(idx).M1_start = M1_start;
                annoSeg(idx).M1_end   = M1_end;
                
                annoSeg(idx).M2_start = M2_start;
                annoSeg(idx).M2_end   = M2_end;
